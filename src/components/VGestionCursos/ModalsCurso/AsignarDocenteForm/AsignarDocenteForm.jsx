...
import "./AsignarDocenteForm.css";
import PropTypes from "prop-types";

function AsignarDocenteModal({
  docente,
  onDocenteUpdated,
  onClose,
  onShowConfirmation,
  ...
  useEffect(() => {
    if (selectedCursoId !== "SELECCIONAR") {
      SubcursoService.getlistarSubcursosPorCurso(selectedCursoId)
        ...
        .catch((error) =>
          console.error("Error al obtener los subcursos:", error)
        );
    } else {
      setSubcursoOptions([
        { label: "Seleccionar SubCurso", value: "SELECCIONAR" },
...
*/
  const handleSave = async (event) => {
    ...
    if (
      selectedCursoId === "SELECCIONAR" ||
      selectedSubcursoId === "SELECCIONAR"
    ) {
      setConfirmationMessage(
        "Debe seleccionar un curso y un subcurso válidos."
      );
  ...
  return (
      ...
      <div className="AsignarDocenteModalContent">
        <h3 className="h3">Asignar Subcurso</h3>
        <ConfirmationModal
        ...
        <form className="AsignarDocenteModalForm" onSubmit={handleSave}>
          ...
          <div className="AsignarDocenteModalButtonContainer">
            <ButtonSubmit
              className="AsignarDocenteModalButtonSave"
              nombre="Guardar"
            />
          </div>
        </form>
